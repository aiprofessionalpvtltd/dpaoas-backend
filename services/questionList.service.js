...
const questionListService = {
    ...
    generateQuestionList: async (req) => {
        try {
            ...
            const [year, month, day] = questionList.houseLayDate.split('-').map(num => parseInt(num));
    ...
    getTextHeight: async (text, options) => {
        ...
        const splitText = font.layout(text, { fontSize, maxWidth, wordBreak: wrap ? 'break-word' : 'normal' });
    ...
    printQuestionList: async (questionList) => {
        try {
            ...
            page.drawText('SENATE OF PAKISTAN', { x: 220, y: yCoordinate + verticalGap - 40, ...boldTextOptions });
            ...
            if (questionList.questionList[0].defferedQuestions === true) {
                page.drawText(`DEFERRED ${questionList.questionList[0].questionCategory} QUESTIONS LIST ${questionList.questionList[0].startListNo} (${questionList.groupDetails.groupNameStarred}) FOR ${questionList.sessionDetails.sessionName} SESSION`, { x: 90, y: yCoordinate + verticalGap - 80, ...textOptions1 });

            }
            else {
                page.drawText(`ADMITTED/DEFERRED ${questionList.questionList[0].questionCategory} QUESTIONS LIST ${questionList.questionList[0].startListNo} (${questionList.groupDetails.groupNameStarred}) FOR ${questionList.sessionDetails.sessionName} SESSION`, { x: 90, y: yCoordinate + verticalGap - 80, ...textOptions1 });
            }

            if (questionList.questionList[0].defferedQuestions === true) {
                page.drawText(`DEFERRED QUESTIONS-                                      ${questionList.questions.length}`, { x: 200, y: yCoordinate + verticalGap - 100, ...textOptions1 });
                ...
                page.drawText(`TOTAL QUESTIONS-                                         ${questionList.questions.length}`, { x: 200, y: yCoordinate + verticalGap - 120, ...textOptions1 });
                ...
                page.drawText(`ADMITTED/DEFERRED QUESTIONS-                                     ${questionList.questions.length}`, { x: 200, y: yCoordinate + verticalGap - 100, ...textOptions1 });
                const lineY = yCoordinate + verticalGap - 104;
            ...
            const ministries = questionList.questions.map(question => question.ministryName);
            ...
            page.drawText(`Ministries (${ministriesString})`, { x: 80, y: yCoordinate + verticalGap - 140, ...textOptions1 });
            ...
            questionList.questions.forEach(async (question, index) => {
                const { questionStatus, memberName, noticeOfficeDiaryDate, noticeOfficeDiaryTime, id, englishText } = question;
                ...
                const neededSpace = await questionListService.estimateTextHeight(englishText, maxWidth, textOptions) + 10; // Additional 60 for spacing and headers
            ...
            // }
                ...
                page.drawText(`*(${questionStatus}) QUESTION NO: SENATOR ${memberName}`, { x: 80, y: currentY, ...boldTextOptions1 });
                page.drawText(`(Notice Received On ${noticeOfficeDiaryDate} at ${noticeOfficeDiaryTime}) QID ${id}`, { x: 80, y: currentY - 20, ...boldTextOptions1 });
                page.drawText(`${englishText}`, { x: 80, y: currentY - 40, maxWidth, wrap: true, ...boldTextOptions1 });
    ...
    editQuestionList: async (questionList, questionIds) => {
        try {
            ...
            throw new Error({ message: error.message || "Error Updating Question List" });
        }
    },    
    ...
    getQuestionListBySessionId: async (sessionId) => {
        try {
            const questionList = await QuestionList.findAll({
                ...
                attributes: ['id', 'questionCategory', 'startListNo', 'questionListStatus', 'listName', 'houseLayDate', 'defferedQuestions', 'questionListStatus']
            })
                ...
                include: [
                    {
                        model: QuestionList,
                        as: 'questionList',
                        attributes: ['id', 'questionCategory', 'fkSessionId', 'fkGroupId', 'startListNo', 'listName', 'houseLayDate', 'defferedQuestions', 'fkUserId', 'questionListStatus']
                    }
                ]
            });
    ...
    getSupplementaryList: async (questionListId) => {
        try {
            ...
            const supplementaryList = await SupplementaryList.findAll({
                where: { fkQuestionListId: questionListId },
                attributes: ['id', 'listName', 'houseLayDate', 'supplementaryListStatus']
            })
