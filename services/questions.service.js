...
const questionsService = {
  ...
  getMemberWiseStatement: async (fromSession, toSession) => {
    try {
      ...
      // // Step 3: Filter only fromSession and toSession
      // const filteredSessions = {};
      // if (sessionMap[fromSession]) {
      //   filteredSessions[fromSession] = sessionMap[fromSession];
      // }
      // if (sessionMap[toSession]) {
      //   filteredSessions[toSession] = sessionMap[toSession];
      // }
      ...
      for (const key in searchCriteria) {
        ...
        if (key === "questionID") {
          queryOptions.where["$questionDiary.questionID$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        if (key === "questionDiaryNo") {
          queryOptions.where["$questionDiary.questionDiaryNo$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        if (key === "keyword") {
        ...
        if (key === "memberPosition") {
          queryOptions.where["$member.governmentType$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        if (key === "Joint Resolution") {
          queryOptions.where["$member.governmentType$"] = {
            [Op.in]: ["Treasury", "Opposition", "Independent", "Anyside"],
          };
          // query["$memberPosition$"] = { [Op.in]: ["Treasury", "Opposition", "Independent", "Anyside"] };
        }
        if (key === "religion") {
          queryOptions.where["$member.religion$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        if (key === "questionCategory") {
          queryOptions.where["$questionCategory$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        if (key === "divisions") {
          queryOptions.where["$divisions.id$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        if (key === "groups") {
        ...
        if (key === "noticeOfficeDiaryNo") {
          queryOptions.where["$noticeOfficeDiary.noticeOfficeDiaryNo$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        if (
          searchCriteria.noticeOfficeDiaryDateFrom &&
          searchCriteria.noticeOfficeDiaryDateTo
        ) {
        ...
        if (key === "questionStatus") {
          queryOptions.where["$questionStatus.id$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        if (key === "fileStatus") {
        ...
        if (key === "questionSentDate") {
          queryOptions.where["$questionSentDate$"] = {
            [Op.eq]: searchCriteria[key],
          };
        }
        // if (key === 'memberPosition') {
    ...
    try {
      ...
      if (question) {
        ...
        if (questionDiary) {
          ...
          updatedQuesDiary = await QuestionDiary.update(updatedQuestionDiary, {
            where: { id: question.fkQuestionDiaryId },
          });
        } else {
          ...
          updatedQuesDiary = await QuestionDiary.update(
            updatedQuestionDiaryId,
            { where: { id: question.fkQuestionDiaryId } }
          );
        }
      } else {
      ...
      const updatedNoticeOffice = {
        noticeOfficeDiaryNo: req.noticeOfficeDiaryNo,
        noticeOfficeDiaryDate: moment(req.noticeOfficeDiaryDate).format(
          "YYYY-MM-DD"
        ),
        noticeOfficeDiaryTime: req.noticeOfficeDiaryTime,
        businessType: "Question",
      ...
      await NoticeOfficeDairy.update(updatedNoticeOffice, {
        where: { businessId: questionId },
      });
      ...
      const updatedQuestion = await Questions.update(questionFileData, {
        where: { id: questionId },
      });
      if (updatedQuestion) {
  ...
  sendToQuestion: async (req, questionId) => {
    try {
      ...
      throw {
        message: error.message || "Error Sending Question To Question Branch!",
      };
    }
  },
  ...
  reviveQuestion: async (req, question) => {
    try {
      ...
      const noticeDiary = await NoticeOfficeDairy.update(updatedNoticeOffice, {
        where: { businessId: question.id },
      });
      ...
      const updatedQuesDiary = await QuestionDiary.update(
        updatedQuestionDiary,
        { where: { id: question.fkQuestionDiaryId } }
      );
  ...
  sendForTranslation: async (questionId) => {
    try {
      ...
      throw {
        message: error.message || "Error Sending Question For Translation!",
      };
    }
  },
  ...
  getQuestionHistories: async (questionId) => {
    try {
      ...
      const response = {
        questionStatusHistory:
          questionStatusHistory.length > 0
            ? questionStatusHistory
            : "No status history data found",
        questionRevival:
          questionRevival.length > 0
            ? questionRevival
            : "No revival data found",
        questionDefer:
          questionDefer.length > 0 ? questionDefer : "No defer data found",
        questionFileHistory:
          questionFileHistory.length > 0
            ? questionFileHistory
            : "No file history data found",
      };
  ...
  deleteQuestion: async (req, questionId) => {
    try {
      ...
      const deletedQuestion = await Questions.findOne({
        where: { id: questionId },
      });
  ...
  getQuestionGroupDiary: async (session, questionCategory, group) => {
    try {
      ...
      return {
        ...
        groupNameUnstarred: groupDetails
          ? groupDetails.groupNameUnstarred
          : null,
  ...
  getQuestionsSummary: async (fromSession, toSession) => {
    try {
      ...
      const modifiedResponse = {
        questionStatusCounts: questionStatusRows.concat({
  ...
  questionDiaryNumberGenerate: async () => {
    try {
        ...
        include: [
          {
            model: NoticeOfficeDairy,
            as: "noticeOfficeDiary",
            attributes: ["noticeOfficeDiaryNo", "noticeOfficeDiaryDate"],
          },
        ],
        order: [["createdAt", "DESC"]],
      });
