...
const fileController = {
    ...
    createFile: async (req, res) => {
        try {
            ...
            logger.info(`fileController: createFile body ${JSON.stringify(req.body)}`)
    ...
    findFilesByRegisterId: async (req, res) => {
        try {
            logger.info(`fileController: findFilesByRegisterId id ${JSON.stringify(req.params.id)} and query ${JSON.stringify(req.query)}`)
            const fileRegisterId = req.params.id;
            // const currentPage = req.query.currentPage;
            // const pageSize = req.query.pageSize
            const mainHeadingNumber = req.query.mainHeadingNumber;
            const { count, files } = await FileService.findFilesByRegisterId(fileRegisterId, mainHeadingNumber)
            if (files.length === 0) {
                return res.status(200).send({
                    success: true,
                ...
                return res.status(200).send({
                    success: true,
                    message: `Files fetched successfully for File Register ${fileRegisterId}`,
                    data: { files, count, },
                })
            }
        }
