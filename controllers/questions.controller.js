...
const questionsController = {
  ...
  createQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: createQuestion body ${JSON.stringify(req.body)}`
      );
      ...
      const existingImages = existingQuestion
      ...
      if (updatedQuestion) {
        updatedQuestion.questionImage = updatedImages.map((imageString) =>
      ...
      return res.status(500).send({
        ...
        message:
          error.message ||
          "An error occurred during the question creation process.",
      });
    }
  },
  ...
  getAllQuestions: async (req, res) => {
    try {
      logger.info(
        `questionsController: getAllQuestions query ${JSON.stringify(
          req.query
        )}`
      );
      console.log("Full query parameters:", req.query);
      const userDesignation = req.query.designationName.trim();
      const allowedRoles = ["Assistant Director"];
      if (!allowedRoles.includes(userDesignation)) {
        return res.status(403).send({
          success: false,
          message:
            "Unauthorized: You do not have permission to view these questions.",
        });
      }
      // const branchId = parseInt(req.query.branch);     
      //   const allowedRoles = ["Admin"];
      //   if (!allowedRoles.includes(userDesignation) || branchId !== 1) {
      //     return res.status(403).send({
      //       success: false,
      //       message:
      //         "Unauthorized: You do not have permission to view these questions.",
      ...
      const questionSentStatus = req.query.questionSentStatus
        ? req.query.questionSentStatus
        : null;
      const { count, totalPages, questions } =
  ...
  // getAllQuestions: async (req, res) => {
  //   try {
  //     const userDesignation = req.query.designationName || 'Admin';
  //     if (userDesignation !== 'Admin') {
  //       return res.status(403).send({
  //         success: false,
  //         message: 'Unauthorized: Only Admin can view these questions.'
  //       });
  //     }

  //     // Continue processing for 'Admin' users...
  //   } catch (error) {
  //     logger.error(error.message);
  //     return res.status(400).send({
  //       success: false,
  ...
  getTodaysQuestions: async (req, res) => {
    try {
      logger.info(
        `questionsController: getTodaysQuestions query ${JSON.stringify(
          req.query
        )}`
      );
      ...
      const questionSentStatus = req.query.questionSentStatus
        ...
        : null;
      const { count, totalPages, questions } =
        await questionsService.getTodaysQuestions(
          currentPage,
          pageSize,
          currentDate,
          questionSentStatus
  ...
  getAllQuestionsInNotice: async (req, res) => {
    try {
      logger.info(
        `questionsController: getAllQuestionsInNotice query ${JSON.stringify(
          req.query
        )}`
      );
      const currentPage = parseInt(req.query.currentPage);
      const pageSize = parseInt(req.query.pageSize);
      const { count, totalPages, questions } =
        await questionsService.getAllQuestionsInNotice(currentPage, pageSize);
      if (questions.length === 0) {
        logger.info("No data found on this page!");
        return res.status(200).send({
          success: true,
  ...
  getQuestionGroupDiary: async (req, res) => {
    try {
      logger.info(
        `questionsController: getQuestionGroupDiary query ${JSON.stringify(
          req.query
        )}`
      );
      ...
      const groupDiary = await questionsService.getQuestionGroupDiary(
        session,
        questionCategory,
        group
      );
  ...
  getUnderProcessQuestions: async (req, res) => {
    try {
      logger.info(
        `questionsController: getQuestionUnderProcess query ${JSON.stringify(
          req.query
        )}`
      );
      const session = req.query.session;
      const questionCategory = req.query.questionCategory;
      const group = req.query.group;
      const underProcessQuestions =
        await questionsService.getUnderProcessQuestions(
          session,
          questionCategory,
          group
        );
  ...
  getQuestionsSummary: async (req, res) => {
    try {
      logger.info(
        `questionsController: getQuestionsSummary query ${JSON.stringify(
          req.query
        )}`
      );
      const fromSession = req.query.fromSession;
      const toSession = req.query.toSession;
      const questionsSummary = await questionsService.getQuestionsSummary(
        fromSession,
        toSession
      );
      if (questionsSummary) {
  ...
  getDeferredQuestions: async (req, res) => {
    try {
      logger.info(
        `questionsController: getDeferredQuestions query ${JSON.stringify(
          req.query
        )}`
      );
      ...
      const { count, totalPages, questions } =
        await questionsService.getDeferredQuestions(currentPage, pageSize);
      if (questions.length > 0) {
  ...
  getDeletedQuestions: async (req, res) => {
    try {
      logger.info(
        `questionsController: getDeletedQuestions query ${JSON.stringify(
          req.query
        )}`
      );
      if (Object.keys(req.query).length !== 0) {
        ...
        const pageSize = req.query.pageSize;
        const { count, totalPages, questions } =
          await questionsService.getDeletedQuestions(
            searchCriteria,
            currentPage,
            pageSize
          );
  ...
  removeQuestionsFromList: async (req, res) => {
    try {
      logger.info(
        `questionsController: removeQuestionsFromList id ${JSON.stringify(
          req.params.id
        )}`
      );
      ...
      if (!questionInList && !questionInSupList) {
        logger.info(
          "Question not found in question list or question supplementary list!"
        );
        return res.status(200).send({
          success: true,
          message:
  ...
  getSingleQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: getSingleQuestion id ${JSON.stringify(
          req.params.id
        )}`
      );
      const questionId = req.params.id;
      const singleQuestion = await questionsService.getSingleQuestion(
        questionId
      );
  ...
  findAllQuestionsByWebId: async (req, res) => {
    try {
      logger.info(
        `questionsController: getallQuestion by web_id ${JSON.stringify(
          req.query.web_id
        )}`
      );
  ...
  getMemberWiseStatement: async (req, res) => {
    try {
      ...
      const fromSession = req.query.fromSession;
      const toSession = req.query.toSession;
      const groups = await questionsService.getMemberWiseStatement(
        fromSession,
        toSession
      );
      if (groups) {
  ...
  getDivisionWiseCategoryCount: async (req, res) => {
    try {
      ...
      const fromSession = req.query.fromSession;
      const toSession = req.query.toSession;
      const groups = await questionsService.getDivisionWiseCategoryCount(
        fromSession,
        toSession
      );
      if (groups) {
  ...
  updateQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: updateQuestion id ${JSON.stringify(
          req.params.id
      ...
      if (updatedQuestion) {
        ...
        const updatedQuestions = await Questions.findOne({
          where: { id: questionId },
        });
        if (updatedQuestions && updatedQuestions.questionImage) {
          updatedQuestions.questionImage = updatedQuestions.questionImage.map(
            (imageString) => JSON.parse(imageString)
          );
        }
  ...
  changeQuestionStatus: async (req, res) => {
    try {
      logger.info(
        `questionsController: changeQuestionStatus body ${JSON.stringify(
          req.body
        )}`
      );

      // Update the question with new information excluding the attachments
      const updatedQuestion = await questionsService.changeQuestionStatus(
        req.body
      );
  ...
  sendToQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: sendToQuestion id ${JSON.stringify(
          req.params.id
        )} and body ${JSON.stringify(req.body)}`
      );
      const questionId = req.params.id;
      ...
      const updatedQuestion = await questionsService.sendToQuestion(
        req.body,
        questionId
      );
  ...
  searchQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: searchQuestion query ${JSON.stringify(req.query)}`
      );
      if (Object.keys(req.query).length !== 0) {
        const searchCriteria = req.query;
        const currentPage = parseInt(req.query.currentPage);
        const pageSize = parseInt(req.query.pageSize);
        const { count, totalPages, questions } =
          await questionsService.searchQuestion(
            searchCriteria,
            currentPage,
            pageSize
          );
  ...
  deferQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: deferQuestion body ${JSON.stringify(req.body)}`
      );
      const questionId = req.params.id;
      const question = await Questions.findByPk(questionId);
      if (!question) {
      ...
      const deferQuestion = await questionsService.deferQuestion(
        req.body,
        question
      );
  ...
  reviveQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: reviveQuestion body ${JSON.stringify(req.body)}`
      );
      const questionId = req.params.id;
      const question = await Questions.findByPk(questionId);
      if (!question) {
      ...
      const revivedQuestion = await questionsService.reviveQuestion(
        req.body,
        question
      );
  ...
  getAllReviveQuestions: async (req, res) => {
    try {
      logger.info(
        `questionsController: getAllReviveQuestions query ${JSON.stringify(
          req.query
        )}`
      );
      const currentPage = parseInt(req.query.currentPage);
      const pageSize = parseInt(req.query.pageSize);
      const { count, totalPages, questions } =
        await questionsService.getAllReviveQuestions(currentPage, pageSize);
      if (questions.length === 0) {
        logger.info("No data found on this page!");
        return res.status(200).send({
          success: true,
  ...
  getSingleReviveQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: getSingleReviveQuestion id ${JSON.stringify(
          req.params.id
        )}`
      );
      const revQuestionId = req.params.id;
      const question = await QuestionRevival.findOne({
        where: { fkQuestionId: revQuestionId },
      });
      if (!question) {
        return res.status(200).send({
          success: true,
      ...
      const singleQuestion = await questionsService.getSingleReviveQuestion(
        revQuestionId
      );
  ...
  getQuestionHistories: async (req, res) => {
    try {
      logger.info(
        `questionsController: getQuestionHistories id ${JSON.stringify(
          req.params.id
        )}`
      );
      const questionId = req.params.id;
      ...
      const questionHistories = await questionsService.getQuestionHistories(
        questionId
      );
  ...
  sendForTranslation: async (req, res) => {
    try {
      logger.info(
        `questionsController: sendForTranslation id ${JSON.stringify(
          req.params.id
        )}`
      );
      const questionId = req.params.id;
      ...
      const updatedQuestion = await questionsService.sendForTranslation(
        questionId
      );
  ...
  deleteQuestion: async (req, res) => {
    try {
      logger.info(
        `questionsController: deleteQuestion id ${JSON.stringify(
          req.params.id
        )}`
      );
      const questionId = req.params.id;
      ...
      const deletedQuestion = await questionsService.deleteQuestion(
        req.body,
        questionId
      );
