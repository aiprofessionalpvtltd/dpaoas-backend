...
const questionsController = {
  ...
  sendToTranslation: async (req, res) => {
    try {
      logger.info(
        `questionsController: sendToTranslation id ${JSON.stringify(
          req.params.id
        )} and body ${JSON.stringify(req.body)}`
      );
      const questionId = req.params.id;
      ...
      const updatedQuestion = await questionsService.sendToTranslation(req, questionId);

      logger.info("Question sent to translation branch successfully!");
      return res.status(200).send({
        success: true,
        message: "Question sent to translation branch successfully!",
        data: updatedQuestion,
      });
    } catch (error) {
      logger.error(error.message);
