...
getAllCasesByBranchId: async (req, res) => {
  ...
  // external-demo
  getCasesByUserAndStatus: async (req, res) => {
  try {
    logger.info(`casesController: getCasesByUserAndStatus params ${JSON.stringify(req.params)} | query ${JSON.stringify(req.query)}`);
    ...
    const { cases, count, totalPages, priorityCounts } = await casesService.getCasesByUserAndStatus(
      userId,
      branchId,
      caseStatus,
      currentPage,
    ...
    return res.status(200).send({
      success: true,
      message: cases.length > 0 ? "Cases fetched successfully!" : "No data found!",
      data: { cases, count, totalPages, priorityCounts },
    });
  } catch (error) {
    logger.error(error.message);
    return res.status(400).send({
