...
const casesController = {
    ...
    createCase: async (req, res) => {
        try {
            logger.info(`casesController: createCase query ${JSON.stringify(req.query)} and body ${JSON.stringify(req.body)}`)
            const fileId = req.params.fileId;
            const createdBy = req.params.createdBy;
            const freshReceiptId = req.params.fkFreshReceiptId ? parseInt(req.params.fkFreshReceiptId) : null;
            const cases = await casesService.createCase(req.body, fileId, createdBy, freshReceiptId);
    ...
    updateCase: async (req, res) => {
        try {
            logger.info(`casesController: updateCase id ${JSON.stringify(req.params.id)} and body ${JSON.stringify(req.body)}`)
            // const fileId = req.params.fileId;
    ...
    getSignatureById: async(req,res) => {
        try {
            logger.info(`casesController: getSignatureById id ${JSON.stringify(req.params.id)}`)
    ...
    assignCase: async (req, res) => {
        try {
            logger.info(`casesController: assignCase FileId ${JSON.stringify(req.params.fileId)}, CaseId ${JSON.stringify(req.params.caseId)} and body ${JSON.stringify(req.body)} and files ${JSON.stringify(req.files)}`)
            const fileId = req.params.fileId;
            const caseId = req.params.caseId;
            const cases = await casesService.assignCase(fileId, caseId, req.files, req.body);
            return res.status(200).send({
                success: true,
    ...
    getCasesHistory: async (req, res) => {
        try {
            logger.info(`casesController: getCasesHistory id ${JSON.stringify(req.params)} and query ${JSON.stringify(req.query)}`)
            const fileId = req.params.fileId;
            const userId = req.params.userId
            const branchId = req.params.branchId
            const currentPage = req.query.currentPage;
            const pageSize = req.query.pageSize;
            const { cases, count, totalPages } = await casesService.getCasesHistory(fileId, userId, branchId, currentPage, pageSize)
            if (cases.length === 0) {
                return res.status(200).send({
                    success: true,
                    message: "No Data Found!",
    ...
    getApprovedCasesHistory: async (req, res) => {
        try {
            logger.info(`casesController: getApprovedCasesHistory query ${JSON.stringify(req.query)}`)
            const fileId = req.query.fileId;
            const userId = req.query.userId
            const branchId = req.query.branchId
            const currentPage = req.query.currentPage;
            const pageSize = req.query.pageSize;
            const { cases, count, totalPages } = await casesService.getApprovedCasesHistory(fileId, userId, branchId, currentPage, pageSize)
    ...
    getSingleCase: async (req, res) => {
        try {
            logger.info(`casesController: getSingleCase id ${JSON.stringify(req.params.id)}`);
            const fileId = req.params.fileId
            const caseId = req.params.caseId;
    ...
    getSingleCaseDetails: async (req, res) => {
        try {
            logger.info(`casesController: getSingleCaseDetails id ${JSON.stringify(req.params)}`);
            const fileId = req.params.fileId
            const caseId = req.params.caseId;
    ...
    deleteCaseAttachment: async (req, res) => {
        try {
            logger.info(`casesController: deleteCaseAttachment for Id ${JSON.stringify(req.params.id)}`);
    ...
    getLowerLevelDesignations: async (req, res) => {
        try {
            logger.info(`casesController: getLowerLevelDesignations id ${JSON.stringify(req.params.id)}`);
            const userId = req.params.id
    ...
    getHigherLevelDesignations: async (req, res) => {
        try {
            logger.info(`casesController: getHigherLevelDesignations id ${JSON.stringify(req.params.id)}`);
            const userId = req.params.id
    ...
    getBranchesByUserLogin: async (req, res) => {
        try {
            logger.info(`casesController: getBranchesByUserLogin id ${JSON.stringify(req.params.id)}`);
            const userId = req.params.id
