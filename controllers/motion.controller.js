...
const MotionController = {
    ...
    getAllMotionsInNotice: async (req, res) => {
        ...
        const { page, pageSize, fileNumber, fkSessionId, noticeOfficeDiaryNo, fkMemberId, fkMinistryId,
            motionId, sessionStartRange, sessionEndRange, fkMotionStatus,
            noticeStartRange, noticeEndRange, englishText, motionWeek, motionType, motionSentStatus, motionSentDate } = query;


        const offset = page * pageSize;
        const limit = pageSize;
        logger.info(`MotionController: getAllMotionsInNotice query ${JSON.stringify(query)}`)
    ...
    getAllMotions: async (req, res) => {
        ...
        const { page, pageSize, fileNumber, fkSessionId, noticeOfficeDiaryNo, fkMemberId, fkMinistryId,
            motionId, sessionStartRange, sessionEndRange, fkMotionStatus,
            noticeStartRange, noticeEndRange, englishText, motionWeek, motionType, motionSentStatus, motionSentDate, memberPosition } = query;
    ...
    findAllMotionsSummary: async (req, res) => {
        try {
            const { fromSessionId, toSessionId, motionType, currentPage, pageSize } = req.query;
            const motionStatusCounts = await motionService.findAllMotionsSummary(fromSessionId, toSessionId, motionType, currentPage, pageSize);
    ...
    createMotionListAndAssociateMotions: async (req, res) => {
        try {
            const payload = req.body;
            const createdMotionList = await motionService.createMotionListAndAssociateMotions(payload);
    ...
    updateMotion: async (req, res) => {
        try {
            ...
            if (result) {
                ...
                const updatedMotions = await motions.findOne({ where: { id: motionId } });
    ...
    sendToMotion: async (req, res) => {
        try {
            logger.info(`MotionController: sendToMotion id ${JSON.stringify(req.params.id)} and body ${JSON.stringify(req.body)}`)
            const motionId = req.params.id;
            const updatedMotion = await motionService.sendToMotion(req.body, motionId);
