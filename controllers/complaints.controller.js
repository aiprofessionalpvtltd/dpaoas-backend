...
const complaintsController = {
    ...
    updateComplaint: async (req,res) => {
        try {
            ...
            await complaintsService.updateComplaint(complaintId, req.body);
            const updatedComplaint = await Complaints.findOne({ where: { id: complaintId } });
            logger.info(`Complaint:${complaintId} updated successfully!`);
            return res.status(200).send({
                success: true,
    ...
    resolveComplaint: async (req, res) => {
        try {
            logger.info(`ComplaintController: resolveComplaint id ${JSON.stringify(req.params.id)} and body ${JSON.stringify(req.body)} and file ${JSON.stringify(req.file)}`)
            const complaintId = req.params.id;
            const complaint = await Complaints.findByPk(complaintId);
            if (!complaint) {
                throw { message: "Complaint Not Found!" };
