...
const filesDashboardController = {
    ...
    // Stats For Sent And Received FRs
    getFRSentAndReceivedStats: async (req,res) => {
        try {
            logger.info(`filesDashboardController: getFRSentAndReceivedStats`)
            const userId = req.params.id
            const stats = await filesDashboardService.getFRSentAndReceivedStats(userId);
            logger.info("FR Sent and Received Stats Retrieved Successfully!");
            return res.status(200).send({
                success: true,
                message: "FR Sent and Received Stats Retrieved Successfully!",
                data: stats,
            });
    ...
    makeNotificationZero: async(req,res) => {
        try {
            logger.info(`filesDashboardController: makeNotificationZero ${JSON.stringify(req.params)}`)
            const notificationId = req.params.notificationId
            const userId = req.params.userId
            await filesDashboardService.makeNotificationZero(notificationId,userId);
            logger.info("Notification Count Decreased!");
            return res.status(200).send({
                success: true,
                message: "Notification Count Decreased!",
            });
