...
const seatingPlanController = {
    ...
    swapSeats: async (req, res) => {
        const { seatNumber1, seatNumber2 } = req.body;
    
        try {
          const result = await seatingPlanService.swapSeats(seatNumber1, seatNumber2);
      ...
      placeSeat: async (req, res) => {
        const { seatNumber1, seatNumber2 } = req.body;
    
        try {
          const result = await seatingPlanService.placeSeat(seatNumber1, seatNumber2);
          res.status(200).json(result);
        } catch (error) {
          res.status(500).send({ 
            success: false,
